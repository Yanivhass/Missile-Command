...
def objective(trial):
    ...
    eps_end = trial.suggest_float("eps_end", 0.001, 0.1, log=True)
    eps_decay = trial.suggest_float("eps_decay", 0.9, 0.999, log=True)
    env = gym.make("gym_missile_command:missile-command-v0")
    env.seed(0)
    env.reset()
    state_size = env.observation_space.shape[0]
    action_size = 24  # flatten_space(env.action_dictionary["attackers"]).shape[0]
    scores = dqn(env=env, state_size=state_size, action_size=action_size, n_episodes=2000,
                 LR=LR, GAMMA=GAMMA, TAU=TAU, BUFFER_SIZE=BUFFER_SIZE, BATCH_SIZE=BATCH_SIZE, UPDATE_EVERY=UPDATE_EVERY,
                 eps_start=1.0, eps_end=eps_end, eps_decay=eps_decay)
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name="DQN_agent_study",
        storage='sqlite:///DQN_agent_study.db',
        load_if_exists=True,
        direction="maximize")
